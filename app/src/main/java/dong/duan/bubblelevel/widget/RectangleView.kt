...
open  class RectangleView : View {
    ...
    public var drawer: RectangleDrawer? = null
    ...
    private fun init(context: Context) {
        point = PointF(0F, 0F)
        drawer = RectangleDrawer(context, isSimple)
    }
    ...
    fun updateOrientation(pitch: Float, roll: Float) {
        if (this.pitch.toInt() != pitch.toInt() || this.roll.toInt() != roll.toInt()) {
            this.pitch = pitch
            this.roll = roll
            point!![width * 0.47f * cos(Math.toRadians((90 - roll).toDouble()))
                .toFloat()] = width * 0.47f * cos(Math.toRadians((90 - pitch).toDouble()))
                .toFloat()
            drawer!!.update(point!!)
            invalidate()
        }
